...
const PriceBox = ({ dt, isModule, locate, secondOption, isTool }) => {
  return (
    <div
      className={` w-70 ${
        isTool ? " h-72" : " h-405 max-h-fit"
      }    mt-8  bg-white border-2   border-gray-500 rounded-lg mx-auto  cursor-default flex flex-col gap-y-2 `}
    >
      ...
      <div className={`${isModule ? "flex justify-center mt-2" : "hidden"}`}>
        <img src={`${dt.image}`} width={60} height={60} alt={dt.image} />
      </div>
      ...
      >
        ...
        <p className=" text-primaryYellow font-bold lowercase mt-6">
          /{dt.monthly}
        </p>
      </div>

      <div className={`${isModule ? " hidden" : 'flex flex-col p-4 h-fit "'}`}>
        {dt.extraInformation &&
          dt.extraInformation.map((data, i) => (
            <div
              className="flex items-center gap-x-2 text-xs font-bold my-2"
              key={i}
            >
      ...
      <div
        className={`${
          isModule ? "flex flex-col" : "hidden"
        } text-center  w-full   `}
      >
        <ButtonPrimary text={dt.button} width={"w-1/2 mb-3"} locate={locate} />
        {dt.buttonSecond && (
          <ButtonPrimary
            text={dt.buttonSecond}
            width={"w-1/2  mb-3"}
            locate={dt.secondLocate}
          />
        )}
      </div>
    </div>
